...
loc="$1"
echo "$loc"
tunnel="ssh -D 8080 -f -C -q -N $loc -p 22"

PID=""
if ! pgrep -f "ssh -D 8080 -f -C -q -N $loc -p 22" 2>&1 > /dev/null; then
	...
	PID=$(pgrep -f "ssh -D 8080 -f -C -q -N $loc -p 22")
fi
...
while true; do
	while pgrep -f "ssh -D 8080 -f -C -q -N $loc -p 22" 2>&1 > /dev/null; do
		sleep 1
	done
